...
class QueryAgentStateBySkillIds extends React.Component {
    componentDidMount() {
    ...
    send = () => {
        ...
        postMsg(this.props, this.props.form.getFieldsValue(), "/ccbms/ws/monitor/queryAgentStateBySkillIds");
    }
    render() {
        const { getFieldDecorator } = this.props.form;
        return (
            <Tabs defaultActiveKey="1">
                <TabPane tab="查询技能下坐席状态 RT_restApI_4" key="1">
                    <Form {...formItemLayout}>
                        <Form.Item label="URL">
                            <Input name="URL" value="/ccbms/ws/monitor/queryAgentStateBySkillIds" disabled />
                        </Form.Item>
                        <Form.Item label={
                            <span>
                                技能队列&nbsp;
                            ...
                            >
                                {getFieldDecorator('skillIds', { rules: [{ required: true, message: 'Please input your systemCode!' }] })(
...
const QueryAgentStateBySkillIdsCard = Form.create({ onValuesChange })(QueryAgentStateBySkillIds);
export default QueryAgentStateBySkillIdsCard;